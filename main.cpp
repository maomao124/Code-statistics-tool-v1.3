...
bool filterpath(string path)
{
	...
	for (int k = 0; k < filterKey.size(); k++)
	{
		if (path.find(filterKey[k]) < path.length())
		{
			return true;
			break;
		}
...
int main()
{
	...
	else
	{
		...
		else
		{
			filter = 1;
			cout << "过滤器过滤关键字：";
			for (int i = 0; i < filterKey.size() - 1; i++)
			{
				cout << filterKey[i] + "、";
			}
			cout << filterKey[filterKey.size() - 1] << endl;
		}
	}
